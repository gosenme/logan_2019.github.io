...
<li><a href="http://www.cnblogs.com/olvo/archive/2012/05/03/2481014.html">UML 类图关系（泛化 、继承、实现、依赖、关联、聚合、组合）</a></li>
</ul>
<p><strong>阅读本系列文章</strong></p>
<p>通过阅读本系列文章，以轻松愉快的方式学习设计模式和编程思想。本系列文章没有阅读的先后顺序，每一章都是单独成文，可从任意一篇文章开始。</p>
<h3 id="python">为什么选择 Python</h3>
...
<h4 id="python-1">大势所趋，Python 已然成风</h4>
<ul>
...
<h3 id="python-2">简单的 Python 基础</h3>
<p>如果已经熟悉 Python 语言，这一部分的内容可直接跳过！</p>
<h4 id="python-3">Python 的特点</h4>
...
<pre><code class="java language-java">int min = x &lt; y ? x : y
</code></pre>
<p>而 Python 是这样的：</p>
<pre><code class="python language-python">min = x if x &lt; y else y
</code></pre>
...
<p>Demo：</p>
<pre><code class="python language-python">age = 18        # int
weight = 62.51  # float
name = "Tony"    # string
print("age:", age)
print("weight:", weight)
...
<h4 id="-7">常用容器</h4>
<h5 id="list"><strong>List（列表）</strong></h5>
...
<p>Demo：</p>
<pre><code class="python language-python">list = ['Thomson', 78, 12.58, 'Sunny', 180.2]
tinylist = [123, 'Tony']
print(list)             # 输出完整列表
