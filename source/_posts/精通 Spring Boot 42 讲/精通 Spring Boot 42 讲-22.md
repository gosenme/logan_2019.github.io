...
<h3 id="">分页查询</h3>
<p>Spring Data JPA 已经帮我们内置了分页功能，在查询的方法中，需要传入参数 Pageable，当查询中有多个参数的时候 Pageable 建议作为最后一个参数传入。</p>
<pre><code class="java language-java">@Query("select u from User u")
Page&lt;User&gt; findALL(Pageable pageable);
...
<pre><code class="java language-java">Slice&lt;User&gt; findByNickNameAndEmail(String nickName, String email,Pageable pageable);
</code></pre>
...
<pre><code class="java language-java">User findFirstByOrderByLastnameAsc();
...
<h3 id="-1">复杂查询</h3>
<p>我们可以通过 AND 或者 OR 等连接词来不断拼接属性来构建多条件查询，但如果参数大于 6 个时，方法名就会变得非常的长，并且还不能解决动态多条件查询的场景。到这里就需要给大家介绍另外一个利器 JpaSpecificationExecutor 了。</p>
...
<pre><code class="java language-java">public interface JpaSpecificationExecutor&lt;T&gt; {
...
<p>运行测试方法后返回：</p>
<pre><code>userInfo: aa-aa@126.com-钓鱼-程序员
</code></pre>
