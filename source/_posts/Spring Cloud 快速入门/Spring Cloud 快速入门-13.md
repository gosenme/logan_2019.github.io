...
<p>实现步骤见下面说明。</p>
<p>1.创建一张数据库表：</p>
<pre><code class="sql language-sql">CREATE TABLE `methodLock` (
...
<p>基于 MySQL 的 InnoDB 引擎，可以使用以下方法来实现加锁操作：</p>
<pre><code class="java language-java">public boolean lock(){
...
<pre><code class="java language-java">public void unlock(){
    connection.commit();
}
</code></pre>
<h3 id="zookeeper">基于 Zookeeper 的分布式锁</h3>
<h4 id="zookeeper-1">ZooKeeper 简介</h4>
